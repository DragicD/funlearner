...
void setup() {
  ...
  pfx.setImg(Particles.TYPE_CLUMP,clumpImg);
...
void draw() {
  ...
  if(focused && gameStarted) {
    ...
    textAlign(CENTER);
    fill(255);
    text("Taxes Spent on Your Plane: $"+nfc(int(gameCost)), width/2, height-20);
  } else {
    fill(255);
    textAlign(LEFT);
    text("(Spacebar) Auto Cannon\n(A Key) Bomb\n(S Key) Cluster\n(D Key) Fire Bomb\n(F Key) Guided Rocket", 10, 20);
    textAlign(CENTER);
    text("HobbyGameDev Plane Demo\nNotgame by Chris DeLeon\nMusic by Kevin MacLeod\nNo winning or losing.\nJust thinking.", width/2, 20);
    textAlign(RIGHT);
    text("Left/Right - Throttle\nUp/Down - Pitch\nA,S,D,F - Bombs\nSpace - Cannon\nTab - Reset", width-10, 20);
  }
}
