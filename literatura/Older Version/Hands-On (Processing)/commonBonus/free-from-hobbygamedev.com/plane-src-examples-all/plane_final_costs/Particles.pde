class Particles {
  ...
  void spawnClump(float atX, float atY,int num,int rval, int gval, int bval) {
    for(int i=0;i<num;i++) {
      ...
      xv[pNum]=random(-2.1,2.1);
      yv[pNum]=random(-4.2,-1.4);

      type[pNum]=TYPE_CLUMP;
      x[pNum]=atX+random(-1.75,1.75);
      y[pNum]=atY+random(-5.6,-2.8);

      life[pNum]=10000;

      maxLife[pNum]=int(rval*0.8);
      ang[pNum]=int(gval*0.8);
      sc[pNum]=int(bval*0.8);
  ...
  void handle(Boolean doingLights) {
    ...
    for(int i=0;i<pNum;i++) {
      ...
      if(x[i]<0 || x[i]>WORLD_SIZE) {
        life[i]=0;
      } else if((type[i]==TYPE_CLUMP || type[i]==TYPE_FIRE) && y[i]>landOffY) {
        int landHeight = land.pixels.length/WORLD_SIZE;
        int h = heightAt(int(x[i]));
        
        if(type[i]==TYPE_CLUMP) {
          if(y[i]>=landOffY+landHeight-1 || (y[i]>=h && alpha(land.pixels[int(x[i])+int((y[i]-landOffY)*WORLD_SIZE)])>=127.0)) {          
            drawSquare(int(x[i]),h-1,3,maxLife[i],int(ang[i]),int(sc[i]));
            life[i]=0;
          }
        } else {          
            y[i] = h;
