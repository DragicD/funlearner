...
void ballMovement()
{
  ...
  ballX += ballSpeedX;
  ballY += ballSpeedY;
  if(ballX <= 0.0) { // off left edge
    ballX = 0.0; // put ball back onto left screen edge
    ballSpeedX *= -1.0; // bounce horizontally
  }
  if(ballX >= SCREEN_W) { // off right edge
    ballX = SCREEN_W; // put ball back onto right screen edge
    ballSpeedX *= -1.0; // bounce horizontally
  }
