     ...
     ballY < BRICK_TOPLEFT_Y + BRICK_H * BRICK_ROWS) {
       ...
       if( brickGrid[hitRow][hitCol] == 1 ) { // brick still there?
         ...
         */
         ...
         if(prevCol != hitCol) {
           boolean brickBlocksHorizontalReflection = (prevCol >= 0 &&
               prevCol < BRICK_COLS &&
               brickGrid[hitRow][prevCol] == 1 );
           if(brickBlocksHorizontalReflection == false) {
             ballSpeedX *= -1.0;
           }
         }
         if(prevRow != hitRow) {
           boolean brickBlocksVerticalReflection = (prevRow >= 0 &&
               prevRow < BRICK_ROWS &&
               brickGrid[prevRow][hitCol] == 1 );
           if(brickBlocksVerticalReflection == false) {
             ballSpeedY *= -1.0;
           }
         }
       }
